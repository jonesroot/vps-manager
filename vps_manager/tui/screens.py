...
class ServerModalScreen(ModalScreen[Server | None]):
        ...
        with Vertical(id="dialog-container"):
            ...
            with ScrollableContainer():
                with Grid(classes="form-grid"):
                    ...
                    # 7. Private Key Path (Conditional)
    ...
    def on_mount(self) -> None:
        """Synchronize field visibility with initial auth method."""
    ...
    def action_save(self) -> None:
        ...
        # Construct candidate Server instance
        try:
            server_id = self._editing_server.id if self._editing_server else None
            new_server = Server(
                id=server_id if server_id else Server(alias="x", host="1.1.1.1").id,
                alias=alias_val,
                host=host_val,
                port=port_val,
                user=user_val,
                ...
                group=group_val,
                tags=tags_tuple,
                status=self._editing_server.status if self._editing_server else Server(alias="x", host="1.1.1.1").status,
...
class BatchCommandRunnerScreen(ModalScreen[None]):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(id="batch-container"):
            yield Static("PARALLEL BATCH COMMAND RUNNER", classes="batch-title")
            yield Static(
    ...
    def action_execute_batch(self) -> None:
        ...
        # Launch concurrent background execution worker
    ...
    async def _run_batch_worker(self, command: str) -> None:
        ...
        log = self.query_one("#batch-log", RichLog)
        progress = self.query_one("#batch-progress", ProgressBar)
        semaphore = asyncio.Semaphore(10)  # Bound concurrent socket allocations
            ...
            async with semaphore:
                ...
                if result.stdout.strip():
                    for line in result.stdout.splitlines():
                        log.write(f"  [{server.alias}] {line}")
                if result.stderr.strip():
                    for line in result.stderr.splitlines():
                        log.write(f"  [{server.alias}][red] {line}[/red]")
...
class HelpScreen(ModalScreen[None]):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="help-container"):
            yield Static("KEYBOARD CHEATSHEET & NAVIGATION", classes="help-title")
            with Vertical(classes="help-table"):
                for key_combo, desc in self.SHORTCUTS:
                    with Horizontal(classes="help-row"):
                        yield Static(f"[{key_combo}]", classes="help-key")
